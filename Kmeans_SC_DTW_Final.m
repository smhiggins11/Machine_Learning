...
for ii = 1:k
    n = 1;
    while n<size(Cluster_Vectors_Cell{1,ii}{1,1},2)+1
        for a = 1:size(Cluster_Vectors_Cell{1,ii}{1,:},2)
            Cluster_Vector(ii).Silhouette(a,n) = dtw(Cluster_Vectors_Cell{1,ii}{1,1}(:,n), Cluster_Vectors_Cell{1,ii}{1,1}(:,a));
        end
        n = n+1;
    end
end
...
for ii = 1:k
    for b = 1:k
        for a = 1:size(Cluster_Vectors_Cell{1,ii}{1,:},2)
            Cluster_Dist(ii).Between(b,a) = dtw(Cluster_Vectors_Cell{1,ii}{1,1}(:,a), Cluster_Centroid(:,b));
        end
    end
end
...
for ii = 1:k
    n = 1;
    while n<size(Cluster_Vectors_Cell{1,ii}{1,:},2)+1
        for b = 1:k
            for a = 1:size(Cluster_Vectors_Cell{1,b}{1,:},2)
                Silhouette(b,ii).Cluster(n,a) = dtw(Cluster_Vectors_Cell{1,ii}{1,:}(:,n), Cluster_Vectors_Cell{1,b}{1,:}(:,a));
            end
        end
        n = n+1;
    end
